...
public class CoreConfigScreen{
    public static MutableComponent translateable(String name){
        return NobodyLibConfigScreen.translateable("core."+name);
    }
    public static void init(ConfigCategory core,Screen parent,ClientConfig config,ConfigBuilder builder,ConfigEntryBuilder entryBuilder){
        SubCategoryBuilder debug=entryBuilder.startSubCategory(translateable("debug"));
        debug.setExpanded(false);
        core.addEntry(debug.build());
    }
}